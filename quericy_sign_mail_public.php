...
<div class="alert alert-active">
    ...
    <div class="container">
        <div class="row">
            <p>用户名称：<a href="http://tieba.baidu.com/"><?php echo $userinfo['name'] ?></a></p>
    ...
    <table class="table table-striped">
        ...
        while ($tieba = $m->fetch_array($query)) {
            ...
            $out_put_str = '<tr class="alert alert-' . $style_str . '"><td>' . ($tieba[$zt] ? $tieba[$zt] : '-') . '</td><td><a href="http://tieba.baidu.com/f?kw=' . urlencode($tieba['tieba']) . '" target="_blank">' . $tieba['tieba'] . '</a></td><td>' . '<span class="glyphicon glyphicon-' . $status_icon . '"></span> ' . $status_str . '</td><td>' . $msg . '</td></tr>';
            echo $out_put_str;
        }
